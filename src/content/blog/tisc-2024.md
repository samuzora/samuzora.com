...
This challenge was a really interesting sandbox escape challenge. We're given
quite a few binaries:
...
also developed by Microsoft. Under the hood it uses `snmalloc`, which is a
relatively new allocator that promises high levels of security compared to
glibc's `dlmalloc` (or `ptmalloc2`). You can read their paper on the development
and main features of the allocator
[here](https://github.com/microsoft/snmalloc/blob/main/snmalloc.pdf).

In another [useful set of
documents](https://github.com/microsoft/snmalloc/tree/main/docs/security), the
developers outline the security features of `snmalloc`. I'll give a brief
