...
all chunks used and hence become inactive until a chunk in the page is freed). There are also different pages for
different purposes, which the kernel uses in order to improve security (it has lots of space for these pages anyway).
For example, there is `kmalloc-256` and `kmalloc-cg-256`, where chunks allocated with `GFP_KERNEL_ACCOUNT` are stored in
`cg`, and normal chunks are stored in the general-purpose page type `kmalloc-256`. This provides even more separation
between chunks used for different purposes. Lastly, the code path taken to allocate a chunk is also important, as it is
used as a seed to determine which of the many `kmalloc-256` pages to allocate to. So it's quite confusing and
unpredictable where our chunks will end up (other than knowing the type of cache).
...
The idea is to use the page freeing mechanism to free the `kmalloc-256` cache containing our UAF object, then
re-allocate the page and make sure it's used for `kmalloc-cg-256`. That way, the old page that contained our UAF object
will now be used to store `msg_msg`, and there will be a `msg_msg` that overlaps with the UAF object! `msg_msg` is quite
ideal here as all the fields in `chemical` will be covered by `msg_msg` input buffer, allowing us to mess around more
with the chemical struct.

So, we want to fully control the page containing our UAF object, such that we can free all the chunks in it and cause it
to be reused for `msg_msg`. One way to do this is to pad the object with `timerfd_ctx` chunks before and after, so that
the entire page will be filled up. However, due to the unpredictability of slub allocator, we won't know exactly at
which offset in the active page our object is allocated. In addition, because of cache randomization, it's not even
guaranteed that we only need 8 before and after our object in order to fully control a page.

But kernel memory is cheap, and we can spam as many chunks as we want. By spraying, we will greatly increase the chances
of complete page allocation. For some reason, only certain spray sizes actually work and it's quite random. In the end,
...
After this, we can free all the `timerfd_ctx` structs, and hopefully the page containing UAF object will be unallocated
as well. Now, we just need to spray lots of `msg_msg` objects to reclaim the page and complete the cross-cache attack. A
good practice is to place the queue index in `msg_msg` so we can identify the `msg_msg` that overlaps our object. After
doing this spray, we have successfully crossed the boundary from `kmalloc-256` to `kmalloc-cg-256`!
...
I used the following script to send the binary to remote:
